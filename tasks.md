Project Root: db-converse-desktop/
Pre-requisites for the LLM:
Assume Python 3.9+ is installed.
Assume pip is available.
The LLM should create files and folders as specified.
...
Action:
Create the main project directory: db-converse-desktop/
Inside db-converse-desktop/, create a Python virtual environment (e.g., python -m venv .venv).
Create requirements.txt with initial dependencies:
customtkinter
python-dotenv
Use code with caution.
Create .gitignore with common Python and venv entries (e.g., /.venv, **pycache**/, \*.pyc, .env).
End: db-converse-desktop/ directory with .venv/, requirements.txt, .gitignore.
...
Start: requirements.txt installed.
Action:
Create db-converse-desktop/app/ directory.
Create db-converse-desktop/app/**init**.py (can be empty).
Create db-converse-desktop/app/main.py with:
...
self.geometry("800x600") # Potentially set appearance mode early # ctk.set_appearance_mode("System") # Light, Dark, System # ctk.set_default_color_theme("blue") # blue, green, dark-blue
...
Use code with caution.
Python
End: app/main.py created.
Test: Run python app/main.py from within db-converse-desktop/. An empty CustomTkinter window titled "DB-Converse" should appear.
Task 1.3: Configuration for API Key (Google Gemini)
Start: app/main.py exists.
Action:
Create db-converse-desktop/.env.example with: GOOGLE_API_KEY="YOUR_GEMINI_API_KEY_HERE"
Create db-converse-desktop/app/core/ directory.
Create db-converse-desktop/app/core/**init**.py (empty).
Create db-converse-desktop/app/core/config.py with:
import os
...
Use code with caution.
Python
End: .env.example and app/core/config.py created.
Test: Create a .env file (copy from .env.example and add a dummy key), then run python -c "from app.core import config; print(config.GOOGLE_API_KEY)". It should print the dummy key or the warning if .env is misconfigured.
...
Test: Run pip install -r requirements.txt again.
Task 2.2: Basic Database Service (db_service.py) - Connection Function
Start: mysql-connector-python installed.
Action:
Create db-converse-desktop/app/services/ directory.
Create db-converse-desktop/app/services/**init**.py (empty).
Create db-converse-desktop/app/services/db_service.py with:
...
Use code with caution.
Python
End: app/services/db_service.py with connect_to_db and disconnect_from_db.
Test: Manually create a small test script or use Python interactive mode:
...
Start: db_service.py has connect_to_db.
Action:
Create db-converse-desktop/app/ui/ directory.
Create db-converse-desktop/app/ui/**init**.py (empty).
Create db-converse-desktop/app/ui/connect_dialog.py with:
import customtkinter as ctk
...
Use code with caution.
Python
Run app/main.py. The dialog should appear, and after filling and clicking "Connect", details should be printed.
Task 2.4: Integrate Connection Dialog with db_service in Main App
Start: ConnectDialog and db_service.connect_to_db exist.
Action:
Create db-converse-desktop/app/core/app_state.py to hold the connection:
...
if **name** == "**main**":
app = App()
app.mainloop() # Ensure disconnection on app close
if current_app_state.db_connection:
db_service.disconnect_from_db(current_app_state.db_connection)
Use code with caution.
Python
End: main.py uses ConnectDialog and db_service to establish and display connection status. app_state.py created.
Test: Run app/main.py. Click "Connect to Database", enter valid credentials for your test DB. The status label should update to "Connected to [your_db_name]". Try with invalid credentials; status should be "Connection Failed".
...
End: requirements.txt updated.
Test: Run pip install -r requirements.txt.
...
Action: Create db-converse-desktop/app/services/nlp_service.py with:
...
try:
response = model.generate_content(prompt_text) # Basic error handling for response structure (may need refinement based on Gemini SDK)
...
- **End:** `app/services/nlp_service.py` with basic Gemini interaction. \* **Test:**python

# In Python interactive or a test script, after activating venv and ensuring .env is set up
...
Use code with caution.
Python
End: db_service.py can fetch table names.
...
Use code with caution.
Python
End: db_service.py can execute SELECT queries.
...
Start: nlp_service.nl_to_sql_basic and db_service.execute_query exist.
Action:
Create db-converse-desktop/app/ui/converse_frame.py:
import customtkinter as ctk
...
Use code with caution.
Python
Integrate ConverseFrame into app/main.py:
...
Use code with caution.
Python
End: ConverseFrame created and displayed in main_window.
Test: Run app/main.py. Connect to your DB. Type a simple NL query (e.g., "list all entries from test_mvp"). See if SQL is generated and results are displayed. Try a query that might fail.
...
Action: Create db-converse-desktop/app/services/chart_service.py:
...
Use code with caution.
Python
End: chart_service.py with generate_bar_chart_figure.
Test:
...
Start: chart_service.generate_bar_chart_figure exists.
Action:
Create db-converse-desktop/app/ui/widgets/ directory.
Create db-converse-desktop/app/ui/widgets/__init__.py (empty).
Create db-converse-desktop/app/ui/widgets/chart_widget.py:
import customtkinter as ctk
...
Start: ChartWidget and chart_service.generate_bar_chart_figure exist. db_service.get_table_names exists.
Action:
Create db-converse-desktop/app/ui/dashboard_frame.py:
import customtkinter as ctk
...
Use code with caution.
Python
Integrate DashboardFrame into app/main.py. For MVP, let's use a TabView.
# Add to imports in main.py:
...
Use code with caution.
Python
And to ensure the dashboard attempts to load on startup if already connected (or to show the "not connected" message):
# In App class __init__, after creating dashboard_frame:
# self.dashboard_frame.load_dashboard_data() # Trigger initial load attempt
# Better: call it after the connect button is pressed and successful.
# Or if you implement loading saved connections.
# For now, user must click "Refresh Dashboard" or reconnect.
# OR, add to open_connect_dialog as shown above.

# Ensure the status label is updated correctly when connection fails or is established
# In open_connect_dialog:
# ...
# if conn:
#     ...
#     self.connection_status_label.configure(text=f"Status: Connected to {details['database']}")
...
Use code with caution.
Python
End: DashboardFrame shows a bar chart of table row counts. UI uses TabView.
Test: Run app/main.py. Connect to DB. Go to "Dashboard" tab. Click "Refresh Dashboard". A bar chart showing row counts for tables in your test DB should appear.
